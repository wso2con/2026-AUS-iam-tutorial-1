...
async function exchangeAsgardeoTokenForClaude(asgardeoAccessToken: string): Promise<{ accessToken: string; expiresIn: number }> {
    const body: Record<string, string> = {
        grant_type: "urn:ietf:params:oauth:grant-type:jwt-bearer",
        assertion: asgardeoAccessToken,
    ...
    if (wifConfig.workspaceId) {
        body.workspace_id = wifConfig.workspaceId;
    }
    ...
    if (!response.ok) {
        ...
        throw new Error(`Anthropic WIF token exchange failed: ${response.status} ${responseText}`);
    }
...
async function createAgent() {
    logger.info("Starting Wayfinder AI agent with Asgardeo, Anthropic WIF, and LangChain");
    ...
    const asgardeoTokenExpiresAtMs = getAgentTokenExpiresAtMs(agentToken);

    logger.info("Exchanging Asgardeo token for Anthropic WIF access token");
    const wifToken = await exchangeAsgardeoTokenForClaude(agentToken.accessToken);
    const wifExpiresAtMs = Date.now() + (wifToken.expiresIn * 1000);

    // Use the shorter of the two token lifetimes as the refresh trigger
    const tokenExpiresAtMs = Math.min(asgardeoTokenExpiresAtMs, wifExpiresAtMs);

    // Each agent instance gets its own model bound to the WIF token for that session.
    // createClient bypasses ChatAnthropic's API key validation; the Anthropic SDK
    // sends only Authorization: Bearer <wifToken> (no x-api-key header).
    const model = new ChatAnthropic({
        model: process.env.CLAUDE_MODEL_NAME || "claude-opus-4-7",
        createClient: () => new Anthropic({ authToken: wifToken.accessToken }),
    });
    ...
    }, "Agent ready with fresh Asgardeo and Anthropic WIF tokens");
...
async function runAgentServer() {
    ...
    async function getRuntime() {
        if (isRuntimeNearExpiry(runtime)) {
            logger.info("Refreshing agent tokens before MCP tool call");
    ...
    const handleConnection = (socket: Duplex) => {
        ...
        sendJson(socket, {
            type: "ready",
            message: "Connected to the Asgardeo Claude AI agent.",
        });
